...
namespace Zigurous.Architecture
{
    ...
    public static class Compare
    {
             ...
             where T : IEquatable<T>
        {
            ...
            for (int i = 0; i < iterations; i++)
            {
                ...
                if (log && logIndividual) {
                    Log.Message("[Compare]", $"{resultFoo} vs {resultBar} | {(equal ? "Equal" : "Not Equal")}");
                }
            }
            ...
            if (log) {
                Log.Message("[Compare]", $"{amountEqual}/{iterations} ({(percentEqual * 100f).ToString()}%) equal results");
            }
             ...
             where T : IEquatable<T>
        {
            ...
            if (log) {
                Log.Message("[Compare]", $"{amountNotEqual}/{iterations} ({(percentNotEqual * 100f).ToString()}%) not equal results");
            }
